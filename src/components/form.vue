<template lang="html">
   <div id="formView">
      <form id="contactForm" class="contact-form" v-if="!submitted">
         ...
         <input type="text" name="Nimi" v-model="contactForm.submittedName" v-validate="'required'" :class="{'input': true, 'is-danger': errors.has('Nimi') }"/> <!-- Andmed väljast loetakse contactForm vastavasse muutujasse. Lisatud on if-lause, kui on probleem valideerimisel, lisatakse vastav klass. -->
         <span v-show="errors.has('Nimi')" class="help is-danger">{{ errors.first('Nimi') }}</span> <!-- Veateate tekst, mis ilmub, kui v-validate avastab vea. -->
         <br>
         ...
         <!-- Andmed väljast loetakse contactForm vastavasse muutujasse. Lisatud on if-lause, kui on probleem valideerimisel, lisatakse vastav klass. Lisatud on custom reegel, mis kontrollib, kas e-mail on sisestatud. -->
         <span v-show="errors.has('Telefon')" class="help is-danger">{{ errors.first('Telefon') }}</span> <!-- Veateate tekst, mis ilmub, kui v-validate avastab vea. -->
         <br>
         ...
         <input type="email" name="Email" v-model="contactForm.submittedEmail" v-validate="emailRules" :class="{'input': true, 'is-danger': errors.has('Email') }"/>
                  <!-- Andmed väljast loetakse contactForm vastavasse muutujasse. Lisatud on if-lause, kui on probleem valideerimisel, lisatakse vastav klass. Lisatud on custom reegel, mis kontrollib, kas telefoninumber on sisestatud. -->
         <span v-show="errors.has('Email')" class="help is-danger">{{ errors.first('Email') }}</span> <!-- Veateate tekst, mis ilmub, kui v-validate avastab vea. -->
         <br>
         ...
         <span v-show="errors.has('Aadress')" class="help is-danger">{{ errors.first('Aadress') }}</span> <!-- Veateate tekst, mis ilmub, kui v-validate avastab vea. -->
         <br>
         ...
         <span v-show="errors.has('Maakond')" class="help is-danger">{{ errors.first('Maakond') }}</span> <!-- Veateate tekst, mis ilmub, kui v-validate avastab vea. -->
         <br>
         ...
         <div class="additional-form" v-if="contactForm.newAddress==='true'">
            ...
            <span v-show="errors.has('Lisa-aadress')" class="help is-danger">{{ errors.first('Lisa-aadress') }}</span> <!-- Veateate tekst, mis ilmub, kui v-validate avastab vea. -->
            <br>
            <label class="field-label" for="">Küla/alevik</label>
            ...
            <span v-show="errors.has('Linn')" class="help is-danger">{{ errors.first('Linn') }}</span> <!-- Veateate tekst, mis ilmub, kui v-validate avastab vea. -->
...
export default {
   computed: {
    emailRules() {   // Valideerimisreegel e-post lahtrile, mis kontrollib, kas telefoninumber on sisestatud
      return this.contactForm.submittedPhone.length ? '' : 'required|email';
   },
   phoneRules() { // Valideerimisreegel telefoninumber lahtrile, mis kontrollib, kas e-mailiaadress on sisestatud
...
/* Muutujad. Muuda, et kiiresti re-brändida. */
$main: #8470E5;
$secondary: #13B2BB;
$input-bg: #ccc;
$borders: #ddd;
